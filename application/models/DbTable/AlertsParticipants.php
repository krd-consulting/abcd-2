...
class Application_Model_DbTable_AlertsParticipants extends Zend_Db_Table_Abstract {
    protected $_name = 'alertsParticipants';
    
    protected function _processText($ptcpID,$formID,$date,$text) {
        ...
        $ptcpQuery = "SELECT firstName FROM participants WHERE id = $ptcpID";
        $ptcpName = $this->getAdapter()->fetchOne($ptcpQuery);
        
        //alter text.
        
        $v1Text = str_replace('{FORMNAME}', $formName, $text);
        $v2Text = str_replace('{NAME}', $ptcpName, $v1Text);
    ...
    public function getPtcpAlertStatus($pid, $type='all') {
        //type is one of 'all', 'forms', 'custom';
        
        switch ($type) {
          case 'all' : $whereAddOn = '';
              break;
          case 'forms' : $whereAddOn = ' AND formID IS NOT NULL';
              break;
          case 'custom': $whereAddOn = ' AND formID IS NULL';
              break;
        };
        
        $today = date("Y-m-d",time());
        
        $queryText = "SELECT COUNT(*) FROM alertsParticipants
    ...
    public function getPtcpAlerts($pid) {
      ...
      foreach ($alertRows as $alertInstance) {
          ...
          if ($compareDate < $now) {
              ...
              if ($alertID < 3) { // ' Alerts 1 and 2 are reserved for system
                  ...
                  $ptcpID = $alertInstance['participantID'];
                  $alertText = $this->_processText($ptcpID,$formID,$printDate,$alertText);
                  $alertType = 'system';
              } else {
                  $alertType = 'custom';
                  $formID = NULL;
    ...
    public function getList($column,$id) /* $column = 'ptcp' or 'alerts' */ {
        switch ($column) {
            case 'ptcp' : 
    ...
    public function addAlert($alert,$ptcp,$startDate='') {
